...
contract Div {
    ...
    constructor(ITRC20 _match) public {
        owner = msg.sender;
        matchtoken = _match;
        dailyPercent = 10;
    }
    ...
    function freeze(uint256 value) public {
        require(matchtoken.allowance(msg.sender, address(this)) >= value, "invalid request");
        require(matchtoken.balanceOf(msg.sender) >= value, "insufficient balance");
        if(!userExist[msg.sender]) {
            userExist[msg.sender] = true;
            userbase.push(msg.sender);
        }
        matchtoken.transferFrom(msg.sender, address(this), value);
